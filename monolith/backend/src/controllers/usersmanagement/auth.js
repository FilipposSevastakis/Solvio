...
export const buyCreditsController = async (req, res) => {
    try {
        ...
        let user = await Users.findOne({ _id: req.params.userid });
        user.credits = `${parseInt(user.credits) + parseInt(req.body.creditsToBuy)
            }`;
...
export async function authenticationController(cookies) {
    if (!cookies.access_token) {
        return false;
    }
    try {
        jwt.verify(cookies.access_token, process.env.JWT_KEY)
        return true;
    } catch (err) {
        return false;
    }
...
export async function usersPermissionsController(cookies) {
    if (!cookies.access_token) {
        return false;
    }
    try {
        const userInfo = jwt.verify(cookies.access_token, process.env.JWT_KEY);
        let user = await Users.findOne({ _id: userInfo.id });
...
export async function adminsPermissionsController(cookies) {
    if (!cookies.access_token) {
        return false;
    }
    try {
...
export async function getRoleController(userID) {
    try {
...
export async function editPermissionsController(cookies, problemID) {
    if (!cookies.access_token) {
        return false;
    }
    try {
        ...
        try {
            const result = await axios.get(
                `http://localhost:5000/api/submitProblem/getProblemInfo/${problemID}`,
                {
                    headers: {
...
export async function deletePermissionsController(cookies, problemID) {
    if (!cookies.access_token) {
        return false;
    }
    try {
